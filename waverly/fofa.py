...
<<<<<<< HEAD
        ...
        for raw in payload.get("results", []):
            ...
            if isinstance(raw, dict):
                # The API occasionally returns dictionaries when fields are omitted.
                data = {str(key): raw.get(key) for key in raw}
                for field_name in field_list:
                    data.setdefault(field_name, raw.get(field_name))
            elif isinstance(raw, (list, tuple)):
                data = {}
                for idx, value in enumerate(raw):
                    key = field_list[idx] if idx < len(field_list) else str(idx)
                    data[key] = value
            else:
                # Handle scalar responses (e.g. a single field) by mapping to the first
                # configured field name when available.
                key = field_list[0] if field_list else "value"
                data = {key: raw}
